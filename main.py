...
import g4f
import time
from TGBot.database.database import add_answer_gpt
app = FastAPI()
client = Client()
Models_Image = [
...
@app.get("/GPT/{userID}/{prompt}", tags=['Запрос чату гпт'], summary='chatGPT')
def generate_answer_gpt(userID,prompt):
    response = g4f.ChatCompletion.create(
        model="gpt-4o-mini",
        messages=[{"role": "user", "content": prompt}],
    ) 
    print(response)
    add_answer_gpt(userID,response)
